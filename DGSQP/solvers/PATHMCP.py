...
class PATHMCP(AbstractSolver):
    ...
    def _solve_mcp(self, u, l, x0, up, parameters=np.array([])):
        ...
        Main.F_py = lambda z: np.array(self.F(z, x0, up, parameters)).squeeze()
        Main.J_py = lambda z: np.array(self.J(z, x0, up, parameters))
    ...
    def solve(self, states: List[VehicleState], parameters: np.ndarray = np.array([])):
        ...
        # q, G = self.f_lws(u, x0, up, parameters)
        ...
        u, l, converged, feas, comp, stat, msg = self._solve_mcp(u, l, x0, up, parameters)
        ...
        J = np.array(self.f_J(u, x0, up, parameters)).squeeze()
