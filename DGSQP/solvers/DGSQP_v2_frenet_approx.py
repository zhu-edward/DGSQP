...
class DGSQP(AbstractSolver):
    def __init__(self, joint_dynamics: CasadiDecoupledMultiAgentDynamicsModel, 
        ...
        self.approximation_eval = params.approximation_eval
    ...
    def solve(self, states: List[VehicleState], 
        ...
        if len(reference) == 0:
            self.reference = [np.zeros(self.N+1) for _ in range(self.M)]

        if self.approximation_eval == 'once':
            _P, _x = self._evaluate_mpcc(u, x0, self.reference)
        ...
        q, G, g, _, _P = self._evaluate(u, None, x0, up, P=P, hessian=False, x=_x)
        G = G.sparse()
        ...
        phi = float(self.f_phi(u, l, np.maximum(0, g), q, G, g, 1.0, x0, up, _P))
        ...
        while True:
            ...
            Q_i, q_i, G_i, g_i, _, P_i = self._evaluate(u, l, x0, up, P=P, x=_x)
            ...
            if None in du:
                ...
                if self.merit_parameter is None:
                    mu = self._get_mu(u, du, l, dl, s, ds, Q_i, q_i, G_i, g_i, x0, up, P_i)
                else:
            ...
            if m_step:
                ...
                _u = u + du
                _l = l + dl
                _q, _G, _g, _, _P = self._evaluate(_u, _l, x0, up, P=P, hessian=False)
                _s = np.maximum(0, _g)
                
                phi = float(self.f_phi(_u, _l, _s, _q, _G, _g, 1.0, x0, up, _P))
                ...
                if phi <= R:
                    ...
                    # if self.approximation_eval == 'once':
                    #     _P, _ = self._evaluate_mpcc(u, x0, self.reference)
                    #     P = np.concatenate((parameters, _P))

                    u, l, _a, phi = self.line_search(u, du, l, dl, s, ds, x0, up, mu, P)
            ...
            if (not d_step) and (not m_step):
                u, l, _a, phi = self.line_search(u, du, l, dl, s, ds, x0, up, mu, parameters)
                _data.step_size = copy.copy(_a)
                # Convergence via relative tolerance
                if np.linalg.norm(u-u_im1) < xtol and np.linalg.norm(l-l_im1) < ltol:
                    rel_tol_its += 1
    ...
    def _evaluate(self, u, l, x0, up, P: np.ndarray = np.array([]), hessian: bool = True, x=None):
        ...
        if self.approximation_eval == 'always':
            _P, x = self._evaluate_mpcc(u, x0, self.reference)
    ...
    def line_search(self, u, du, l, dl, s, ds, x0, up, mu, P):
        ...
        if self.merit_decrease_condition == 'max':
            ...
            _Q, _q, _G, _g, _, _P = self._evaluate(u, l, x0, up, P=P, hessian=True)
            _phi = float(self.f_phi(u, l, s, _q, _G, _g, mu, x0, up, _P))
            _dphi = float(self.f_dphi(u, du, l, dl, np.maximum(0, _g), _Q, _q, _G, _g, mu, x0, up, _P))
        ...
        for i in range(self.line_search_iters):
            ...
            phi = float(self.f_phi(_u, _l, _s, _q, _G, _g, mu, x0, up, _P))
            R = decrease_condition(_a)
            if self.verbose:
                self.print_method(f'-- Line search step: {i} | merit : {phi:.3f} | reference: {R:.3f} | a: {_a:.3e}')
